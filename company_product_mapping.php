...
<html lang="en">
  <head>
    ...
    <title>Company Product Mapping </title>
...
?>
  ...
  <form action="company_product_mapping_process.php" method="POST">
  <label for="name">Select Company name:</label>
    <select name="company_id" onchange="showCustomer(this.value)" class="form-control">                                 
<?php
$sql ="SELECT * FROM company_details where status='y' AND created_by='$_SESSION[login_inv_user_email]'";  
...
?>
    ...
    <select name="product_id[]" onchange="showCustomer(this.value)" class="form-control" multiple>                                 
<?php
$sql2 ="SELECT * FROM product_details where status='y' AND created_by='$_SESSION[login_inv_user_email]'";  
