...
<?php
  ...
  $sql3="SELECT 
product_id,product_name,product_model,current_rate, td.tr_updated_date_time, 
...
from transaction_details td, customer_details cd, product_details pd where td.tr_product_id = pd.product_id and pd.status ='y' and td.tr_customer_id = cd.customer_id AND td.tr_user_company_id = '$_SESSION[company_id_for_db_update]'";

if($report_filter_select == "daily_rep")
{
  $sql3 = $sql3."AND date (td.tr_updated_date_time) = '$rep_date' group by product_type, pd.product_id ,pd.product_name,product_model,current_rate,td.tr_updated_date_time";
}
else if($report_filter_select == "month_rep")
{
  $sql3 = $sql3."AND month (td.tr_updated_date_time) = '$month' AND year(td.tr_updated_date_time) = '$year' group by product_type, pd.product_id ,pd.product_name,product_model,current_rate,td.tr_updated_date_time";
}
else if($report_filter_select == "year_rep")
{
  $sql3 = $sql3."AND year(td.tr_updated_date_time) = '$year' group by product_type, pd.product_id ,pd.product_name,product_model,current_rate,td.tr_updated_date_time";
}
...
<!--
  ...
  -->
<table border='1' cellspacing='10' cellpadding='20p'>
  <tr> <th>Product Name</th><th>Product Model</th><th>Current Rate</th><th>Total QTY</th><th>Total PRICE</th><th>Updated Date/Time</th></tr>
  <?php
    //echo "<h2>Report 2</h2>";
    if($result=$conn->query($sql3))
    {
