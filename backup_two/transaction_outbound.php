...
echo "<tr><td><input type='checkbox' name='product_id[]' id='checkboxenable$idincrement' value=$row->product_id>&nbsp$row->product_name</td><td>$row->quantity</td>
<td>₹$row->current_rate<input type='hidden' class='iprice' value='$row->current_rate'></td>
<td><input type='number' class='iquantity' onchange='numbox$idincrement()' name='req_quantity[]' value='0' min='0' max='$row->quantity' id='duplicate_value$idincrement'></td>
...
-->
    <center><button id="sub" type="submit" name="submit" class="btn btn-success" onsubmit="eliminatezero()">Submit</button></center>
  </form>
  <!--span id="result"></span>
...
<?php
for($incr=1; $incr<1000; $incr++)
{

?>
  <script>
  function numbox<?php echo $incr; ?> (){
    //var numboxvalue = document.getElementById('duplicate_value<?php //echo $incr; ?>').value; 
    //alert(numboxvalue);
      document.getElementById('checkboxenable<?php echo $incr; ?>').checked = true;
    }
  ...
  function eliminatezero() {
    ...
    if (numboxvalue == 0)
    {
      document.getElementById('duplicate_value<?php echo $incr; ?>').disabled=true;
    }
  }
  </script>
