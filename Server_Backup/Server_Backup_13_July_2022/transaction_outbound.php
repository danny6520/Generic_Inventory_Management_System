...
<table border='1' cellspacing='10' cellpadding='20p'>
      ...
      {
        ?>
         <tr><th>Product Name</th><th>Product Type</th><th>Available Quantity</th><th>MRP</th>
      <?php
      }
      else {
        ?>
...
$sql3 = "SELECT pd.product_id, pd.product_name, pp.quantity, pp.current_rate, pp.mrp, pd.status, pd.created_user_company_id, pp.created_user_company_id, pp.product_pricing_id FROM product_details pd, product_pricing pp WHERE pd.product_id = pp.product_pricing_id AND pd.created_user_company_id = pp.created_user_company_id AND pd.created_user_company_id = '$_SESSION[company_id_for_db_update]' AND pd.status = 'y'";
//echo $sql3;
if ($_SESSION['company_id_for_db_update'] == $bharat_petrolium_company_id)
      {
        ...
        $sql3 = "SELECT pd.product_id, pd.product_name, pd.product_type, pp.quantity, pp.current_rate, pp.mrp, pd.status, pp.product_pricing_id, pd.created_user_company_id, pp.created_user_company_id FROM product_details pd, product_pricing pp WHERE pd.product_id = pp.product_pricing_id AND pd.created_user_company_id = pp.created_user_company_id AND pd.created_user_company_id = '$_SESSION[company_id_for_db_update]' AND pd.status = 'y'";
      }
$random=1;
//echo $sql3;
...
echo "<tr><td><input type='checkbox' name='product_id[]' onclick='enable$random()' value=$row->product_id>&nbsp$row->product_name</td><td>$row->product_type</td><td>$row->quantity</td>
<td>₹$row->mrp<input type='hidden' class='iprice' value='$row->mrp'></td>";
