<?php
session_start();
$login_inv_user_email_for_session = $_SESSION['login_inv_user_email_viewer'];
if($_SESSION['login_inv_user_email_viewer']=$login_inv_user_email_for_session)
{
//start   
    ...
    <ul class="navbar-nav mr-auto">
      ...
      <li class="nav-item dropdown active">
        ...
        <div class="dropdown-menu" aria-labelledby="dropdown06">
          <a class="dropdown-item" href="transaction_outbound_report_viewer.php">Outbound Transaction Report</a>
          <a class="dropdown-item" href="transaction_inbound_report_viewer.php">Inbound Transaction Report</a>
          <a class="dropdown-item" href="company_customer_report_viewer.php">Customer Report</a>
          <a class="dropdown-item" href="company_product_report_viewer.php">Product Report</a>
          <a class="dropdown-item" href="inventory_report_viewer.php">Inventory Report</a>
          <a class="dropdown-item" href="billing_viewer.php">Billing</a>
        </div>
      </li>
      <!--
      <li class="nav-item">
...
<div class="container">
  ...
  <h6>Logged in as: <b><font color="maroon"><?php echo $_SESSION['login_inv_user_email_viewer']; ?></font></b></h6>
  <h6>Your Organization: <b><font color="maroon">
  <?php echo $_SESSION['company_name_for_displaying_viewer']; ?>
  </font></b></h6>

  <hr>
  
  <div id="btntxtalign">
  <a href="transaction_outbound_report_viewer.php"><button type="button" class="btn btn-primary">Show all data</button></a>
</div>
<br>


<div id="example3">
  <form action="transaction_outbound_bill_viewer.php" method="POST">
    

    <label for="name">Select Customer:</label>
    <select name="customer_id" class="form-control">                                 
<?php
//$sql3 ="SELECT cd.customer_name, cd.customer_id, ud.user_email, ud.user_company_id, ccm.company_id, ccm.customer_id FROM customer_details cd, user_details ud, company_customer_mapping ccm WHERE ud.user_email='$_SESSION[login_inv_user_email_viewer]' AND ud.user_company_id=ccm.company_id AND ccm.customer_id=cd.customer_id";  
$sql3 = "SELECT customer_name, customer_id, created_user_company_id FROM customer_details WHERE created_user_company_id = '$_SESSION[company_id_for_db_update_viewer]'";
//echo $sql;
...
if($result=$conn->query($sql4))
    ...
    while($row=$result->fetch_object())
    {
    echo "<tr><td>$row->customer_name</td><td>$row->product_name</td><td>$row->tr_product_quantity</td><td>$row->tr_updated_date_time</td><td>$row->tr_updated_by</td></tr>";
    }
    }
    }
}
