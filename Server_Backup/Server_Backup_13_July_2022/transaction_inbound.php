...
<div class="container">
  ...
  <hr>
  <div id="btntxtalign">
  <a href="transaction_inbound_other_dates.php"><button type="button" class="btn btn-primary">Transaction for other dates</button></a>
</div><br>
  <div id="example3">
    
  <form action="transaction_inbound.php" method="POST">
...
<form action="transaction_inbound_process.php" method="POST">
...
<tr><th>Product Name</th><th>Available Quantity</th><th>Returned Quantity</th><th>Delivered quantity</th><th>Customer Name</th><th>Transaction Date</th></tr>        
<?php
...
$sql3 = "SELECT pd.product_id, pd.product_name, pp.quantity, td.tr_product_id, td.tr_product_quantity, td.tr_id, td.tr_updated_date_time, td.tr_customer_id, cd.customer_id, cd.customer_name FROM product_details pd, transaction_details td, customer_details cd, product_pricing pp WHERE pp.product_pricing_id = pd.product_id AND pd.created_user_company_id = '$_SESSION[company_id_for_db_update]' AND td.tr_user_company_id = '$_SESSION[company_id_for_db_update]' AND td.tr_type = 'o' AND pd.product_id = td.tr_product_id AND cd.customer_id = td.tr_customer_id AND td.tr_customer_id = '$customer_id'";
echo $sql3;
$random=1;
...
<td>$row->tr_product_quantity</td><td>$row->customer_name<input type='hidden' name='customer_id' value='$customer_id'></td><td>".substr("$row->tr_updated_date_time",0,10)."</td></tr>";   
