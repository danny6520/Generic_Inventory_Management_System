...
<div class="container">
  ...
  <br><br>
  <a href="transaction_outbound.php"><button type="button" class="btn btn-primary">Go Back</button></a>
</div><br>
  <div id="example3">
  <form id="myForm" action="transaction_outbound_next_month.php" method="POST">
...
<hr>
    ...
    <form action="transaction_outbound_next_month_process.php" method="POST">
<table border='1' cellspacing='10' cellpadding='20p'>
<tr><th>Product Name</th><th>Available Quantity</th><th>Delivered Quantity<th>Transfering Quantity</th><th>Transfer Date</th><th>Customer Name</th><th>Transaction Date</th></tr>        
<?php
...
$sql3 = "SELECT pd.product_id, pd.product_name, pd.quantity, td.tr_id, td.tr_product_id, td.tr_product_quantity, td.tr_updated_date_time, td.tr_customer_id, cd.customer_id, cd.customer_name FROM product_details pd, transaction_details td, customer_details cd WHERE pd.created_user_company_id = '$_SESSION[company_id_for_db_update]' AND td.tr_user_company_id = '$_SESSION[company_id_for_db_update]' AND td.tr_type = 'o' AND td.tr_type NOT IN ('i') AND pd.product_id = td.tr_product_id AND cd.customer_id = td.tr_customer_id AND td.tr_customer_id = '$customer_id'";
//echo $sql3;
$random=1;
...
while($row=$result->fetch_object())
{
echo "<tr><td><input type='checkbox' name='product_id[]' onclick='enable$random()' value=$row->product_id$row->tr_id><input type='hidden' name='other_dates_tr_id[]' value='$row->tr_id'>&nbsp<label>$row->product_name</label></td>
...
<td><input type='datetime-local' name='transfer_outbound_date[]' id='enabledate$random' class='form-control' disabled></td>
<td>$row->customer_name<input type='hidden' name='customer_id' value='$row->customer_id'></td><td>".substr("$row->tr_updated_date_time",0,10)."</td>
...
<script>
 function enable<?php echo $u; ?>(){
  ...
  document.getElementById("enabledate<?php echo $u; ?>").disabled = "";
  }
